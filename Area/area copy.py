...
for image_counter, image in enumerate(image_array):
    if image_patient_number[image_counter] == "5": # DELETE THIS - TESTING PURPOSES
        ...
        try:
            ...
            # figure_fft = plt.subplot(math.ceil(14/2), 2, temp_counter_patient_5+1)
            ...
            # figure_fft = plt.subplots(figsize = (8,2))
            # temp_title = image_names[image_counter].replace("(5)_C__RECT", "")
            # figure_fft = plt.title(str(temp_title))  
            # figure_fft = plt.plot(xf, np.abs(yf))
            # figure_fft = plt.text(0.15, 1500, "MAX: " + str(round(yf_max, 2)), bbox=dict(facecolor='beige', alpha=0.5))
            # figure_fft = plt.xlim(0, 0.2)
            # figure_fft = plt.ylim(0, 2000)
...
df.to_csv('Area/TEMP_RESULTS_PATIENT5.csv', index=False)
# END OF CODE - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - #